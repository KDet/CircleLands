	...
	public class XCProject : IDisposable
	{
		...
		public void ApplyMod( XCMod mod )
		{
			...
			foreach( string folderPath in mod.folders ) {
				...
				AddFolder( absoluteFolderPath, modGroup, (string[])mod.excludes.ToArray(  ) );
			}


			foreach( string headerpath in mod.headerpaths ) {
				string absoluteHeaderPath = AddXcodeQuotes( Path.Combine( mod.path, headerpath ) );
				AddHeaderSearchPaths( absoluteHeaderPath );
			}

			foreach( string librarypath in mod.librarysearchpaths ) {
				string absolutePath = AddXcodeQuotes(Path.Combine( mod.path, librarypath ));
				AddLibrarySearchPaths( absolutePath );
			}

			if(mod.frameworksearchpath != null)
			{
				foreach( string frameworksearchpath in mod.frameworksearchpath ) {
					string absoluteHeaderPath = AddXcodeQuotes(Path.Combine( mod.path, frameworksearchpath ));
					AddFrameworkSearchPaths( absoluteHeaderPath );
				}
			}
