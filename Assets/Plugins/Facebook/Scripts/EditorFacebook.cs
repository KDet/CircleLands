...
namespace Facebook
{
    class EditorFacebook : AbstractFacebook, IFacebook
    {
        ...
        public void MockLoginCallback(FBResult result)
        {
            ...
            try
            {
                var json = (List<object>) Json.Deserialize(result.Text);
                ...
                var userData = (Dictionary<string, object>) Json.Deserialize(responses[0]);
                var appData = (Dictionary<string, object>) Json.Deserialize(responses[1]);
