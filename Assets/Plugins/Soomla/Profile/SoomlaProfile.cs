...
namespace Soomla.Profile
{
	...
	{
		static SoomlaProfile _instance;
		...
		static private int unreadyProviders;
		...
		public static void Login(Provider provider, string payload="", Reward reward = null) {
			SoomlaUtils.LogDebug (TAG, "Trying to login with provider " + provider);
			SocialProvider targetProvider = GetSocialProvider(provider);
			string userPayload = payload ?? "";
			if (targetProvider == null)
			{
				SoomlaUtils.LogError(TAG, "Provider not supported or not set as active: " + provider);
				return;
			}
