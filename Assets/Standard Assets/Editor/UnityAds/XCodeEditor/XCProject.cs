	...
	public class XCProject : IDisposable
	{
		...
		public PBXDictionary AddFile(string filePath, PBXGroup parent = null, string tree = "SOURCE_ROOT", bool createBuildFiles = true, bool weak = false)
		{
			...
			if(tree.CompareTo("SOURCE_ROOT") == 0) {
				Uri fileURI = new Uri(absPath);
				Uri rootURI = new Uri((projectRootPath + "/."));
			...
			if(fileReference == null) {
				fileReference = new PBXFileReference(filePath, (TreeEnum)Enum.Parse(typeof(TreeEnum), tree));
		...
		{
			PBXGroup modGroup = GetGroup(mod.group);
			...
			foreach(XCModFile libRef in mod.libs) {
				...
				AddFile(MacPath(completeLibPath), modGroup, libRef.sourceTree, true, libRef.isWeak);
			}
			...
			if(mod.frameworks != null) {
				foreach(string framework in mod.frameworks) {
					...
					AddFile(MacPath(completePath), frameworkGroup, "SDKROOT", true, isWeak);
				}
			}
			...
			if(mod.files != null) {
				foreach(string filePath in mod.files) {
					...
					AddFile(MacPath(absoluteFilePath), modGroup);
				}
			}

			if(mod.folders != null) {
				foreach(string folderPath in mod.folders) {
					string absoluteFolderPath = AddXcodeQuotes(Path.Combine(mod.path, folderPath));
					AddFolder(MacPath(absoluteFolderPath), modGroup, (string[])mod.excludes.ToArray());
				}
			}
			...
			if(mod.headerpaths != null) {
				foreach(string headerpath in mod.headerpaths) {
					...
					AddHeaderSearchPaths(AddXcodeQuotes(MacPath(headerpath.Contains("$(SRCROOT)") ? headerpath : absoluteHeaderPath)));
				}
			}

			if(mod.librarysearchpaths != null) {
				foreach(string librarypath in mod.librarysearchpaths) {
					string absolutePath = Path.Combine(mod.path, librarypath);
					AddLibrarySearchPaths(AddXcodeQuotes(MacPath(librarypath.Contains("$(SRCROOT)") ? librarypath : absolutePath)));
				}
			}

			if(mod.frameworksearchpath != null) {
				foreach(string frameworksearchpath in mod.frameworksearchpath) {
					string absoluteHeaderPath = Path.Combine(mod.path, frameworksearchpath);
					AddFrameworkSearchPaths(AddXcodeQuotes(MacPath(frameworksearchpath.Contains("$(SRCROOT)") ? frameworksearchpath : absoluteHeaderPath)));
				}
			}
		...
		public void Consolidate()
		{
			...
			consolidated.Append(buildFiles);
			consolidated.Append(copyBuildPhases);
			consolidated.Append(fileReferences);
			consolidated.Append(frameworkBuildPhases);
			consolidated.Append(groups);
			consolidated.Append(nativeTargets);
			consolidated.Add(project.guid, project.data);
			consolidated.Append(resourcesBuildPhases);
			consolidated.Append(shellScriptBuildPhases);
			consolidated.Append(sourcesBuildPhases);
			consolidated.Append(buildConfigurations);
			consolidated.Append(configurationLists);
